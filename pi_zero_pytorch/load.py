...
def build_converted_state_dict(pi_weights, pz_state):
    ...
    if 'time_mlp_in.weight' in pi_keys:
        pz_state['to_time_cond.1.weight'].copy_(pi_weights.get_tensor('time_mlp_in.weight'))
        pz_state['to_time_cond.1.bias'].copy_(pi_weights.get_tensor('time_mlp_in.bias'))
    if 'time_mlp_out.weight' in pi_keys:
        pz_state['to_time_cond.3.weight'].copy_(pi_weights.get_tensor('time_mlp_out.weight'))
        pz_state['to_time_cond.3.bias'].copy_(pi_weights.get_tensor('time_mlp_out.bias'))

    # Action-time fusion (for PI0 style)
    if 'action_time_mlp_in.weight' in pi_keys:
        if 'to_action_time_fuse.0.weight' in pz_state:
            pz_state['to_action_time_fuse.0.weight'].copy_(pi_weights.get_tensor('action_time_mlp_in.weight'))
            pz_state['to_action_time_fuse.0.bias'].copy_(pi_weights.get_tensor('action_time_mlp_in.bias'))
            pz_state['to_action_time_fuse.2.weight'].copy_(pi_weights.get_tensor('action_time_mlp_out.weight'))
            pz_state['to_action_time_fuse.2.bias'].copy_(pi_weights.get_tensor('action_time_mlp_out.bias'))
